...
# install dependency
RUN apt-get update && \
    apt-get install gosu netcat iputils-ping net-tools wget unzip curl zlib1g libc6 libgcc1 libgssapi-krb5-2 libicu66 libssl1.1 libstdc++6 -y && \
...
COPY  --chown=1001:1001 ./scripts/* /usr/local/bin

RUN wget https://github.com/tModLoader/tModLoader/releases/download/v2023.10.3.0/tModLoader.zip \
    && unzip tModLoader.zip -d ${BASE_PATH}/ \
    && rm tModLoader.zip \
    && chmod +x ${BASE_PATH}/start-tModLoaderServer.sh \
    && chmod +x /usr/local/bin/init-tModLoaderServer.sh \
    && chmod +x /usr/local/bin/get-mod.sh \
    && chmod +x /usr/local/bin/get-mod-version.sh
...
RUN chown -R 1001:1001 /home/tmod
WORKDIR /home/tmod
USER tmod
